...
export class ApiWhatsappRepository implements IWhatsappRepository {
  ...
  async updateTask(task: WhatsAppTask): Promise<void> {
    await apiRequest('/communication/whatsapp/queue', {
      method: 'PUT',
      body: JSON.stringify(task),
    });
  }
  ...
  async deleteTask(id: string): Promise<void> {
    await apiRequest(
      `/communication/whatsapp/queue/${encodeURIComponent(id)}`,
      { method: 'DELETE' },
    );
  }
  ...
  async saveTemplate(template: WhatsAppTemplate): Promise<void> {
    await apiRequest(
      `/communication/whatsapp/templates/${encodeURIComponent(template.id)}`,
      {
        method: 'PUT',
  ...
  async resetTemplates(defaults: WhatsAppTemplate[]): Promise<void> {
    await apiRequest('/communication/whatsapp/templates/reset', {
      method: 'POST',

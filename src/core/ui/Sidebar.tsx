...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onNavigate, isOpen, toggleSidebar, userRole, isPersonalZone, onToggleZone }) => {
  ...
  const workspaceMenuGroups = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: ViewState.STORE_ADMIN, label: 'Inventory', icon: <Store size={18} />, resourceId: 'ops_inventory', roleReq: [UserRole.SUPER_ADMIN, UserRole.OPERATIONS] },
  ...
  return (
    <>
      ...
      <div className={`fixed lg:static inset-y-0 left-0 w-72 bg-slate-900 text-white z-50 transform transition-transform duration-300 lg:transform-none ${isOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col h-full border-r border-slate-800`}>

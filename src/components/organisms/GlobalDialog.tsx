      ...
      >
        <div className="p-6">
          <div className="flex items-start gap-4">
            ...
            <button
              onClick={onCancel}
              className="text-slate-400 hover:text-slate-600 transition-colors -mt-2 -mr-2 p-2 rounded-full hover:bg-slate-50"
            >
              <X size={20} />
            </button>
          </div>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex gap-3 justify-end">
          <button
            onClick={onCancel}
            className="px-4 py-2.5 bg-white border border-slate-300 text-slate-700 font-bold rounded-xl text-sm hover:bg-slate-100 transition-colors shadow-sm"
          >
            {options.cancelLabel || 'Cancel'}
          </button>
          <button
            onClick={onConfirm}
            className={`px-6 py-2.5 text-white font-bold rounded-xl text-sm shadow-lg transition-all transform active:scale-95 ${styles.btnBg}`}
          >
